...
function QueueInterface() {
	...
	skillQueue.doUnlearnedSkillsCache();
	skillQueue.doAvailableSkillsCache(function(x) { displayQueuedItems(); });
		...
		api_call("skills.listLearning", {}, function(e) {
			...
			if (e.learning) {
				for (skillId in e.learning) {
					log("Skill " + skillQueue.unlearnedSkills[skillId].name + " selected externally");
			...
			// Refresh both caches.
			skillQueue.doUnlearnedSkillsCache();
			skillQueue.doAvailableSkillsCache(function(x) {
